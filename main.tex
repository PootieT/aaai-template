...
% 
We also found out that instability does not affect the reconstruction accuracy of SAE.
...
While this is typically less of a problem from the machine learning
perspective where the task is to obtain the accurate autoencoder, its
unstable latent representation poses a great threat to the reasoning ability of
the planners.
% 
...
% 
In a machine learning task, the mapping between images and the binary representation
could be a many-to-many relationship, while the symbolic planning requires a mapping
...
The proposed method has some similarities to the common technique called $\ell_1$ regularization \cite{Goodfellow-et-al-2016},
...
\reftbl{tab:ama2} (right) shows that ZSAEs improve the success ratio over vanilla SAEs.
