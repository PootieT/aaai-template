...
Figure \ref{fig:plateau-noh} plots the number of states with $f(n) = f^*$ (y-axis)
versus the number of states with $f(n) \leq f^*$.
for a set of COUNT standard IPC problem instances from IPC98-IPC11.
Note that there are many instances where a large fraction of the nodes with $f(n) \leq f^*$ have $f(n)=f^*$.
For example, note that in the openstacks domain, all instances are on the y=x line, indicating that almost all states with $f(n) \leq f^*$ have cost $f^*$.
...
\begin{figure}[htb]
 ...
 \caption{
   {\bf Only the plot with no h-tiebreaking here}:
Comparison of the size of the search plateau compared to the
 ...
 Openstacks clearly has the large plateaus.}  \label{fig:plateau-noh}
\end{figure}
