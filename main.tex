...
\todo{For Anna --- 
Since % this paper is a part of the ongoing research, some details are not well formed. also,
I am doing this work both as a class assignment and also as my ``slum-dunk'' research, I decided to add a side note for the nonexperts, in order to optimize the overall effort for both. However, those side notes could also be included in the final version of this paper, if the explanation was found to be also helpful for the reviewer.}
...
The important finding by \citeauthor{helmert2008good} is that even with this intractable and impractical heuristic function, the number of the nodes in the last plateau of the search becomes exponentially large as the problem size increases (\refig{hc-fails}).
...
\begin{figure}[htbp]
 \centering
 \includegraphics{img/hc-fails.png}
...
Another thing we have to note is that in FD, when even the $h$ values are the same, then the second tiebreaking behavior is basically a FIFO order: The planner selects the nodes that is inserted earlier.  \todo{Note for anna --- FIFO order = first-in, first-out order, LIFO --- last-in, first-out order}
