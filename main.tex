...
Given a dataset $\mathcal{X}=\{x^{(1)},\dots,x^{(I)}\}$,
...
where $x\in \mathcal{X}$ corresponds to a visual observation of the environment, and $b\in\{0,1\}^N$ corresponds to its latent representation.
...
\relsize{-1}
\begin{align}
\label{vae-obj} \sum_{x\in \mathcal{X}} \mathbb{E}_{q(b\mid x)}\left[-\log p(x\mid b)\right] + \mathrm{KL}(q(b\mid x) \parallel p(b)),
\end{align}
}
...
\relsize{-1}
\begin{align*}
 &{\mathrm{KL}}(q(b\mid x) \parallel p(b)) \\
= & - \sum_{n=1}^N \sum_{k\in\{0,1\}}q(b_{n}=k \mid x) \log\frac{p(b_{n}=k)}{q(b_{n}=k \mid x)}\\
=& - \sum_{n=1}^N \sum_{k\in\{0,1\}}q(b_{n}=k \mid x) \left(\log \frac{1}{2} - \log{q(b_{n}=k \mid x)}\right)\\
=& - \sum_{n=1}^N H(q(b_n\mid x)) + \mathrm{const.} = -H(q(b \mid x)) + \mathrm{const.},
\end{align*}
}
...
\label{loss+ent} =&\brackets{\text{\small the original GS VAE loss}}      + 2 H(q(b \mid x)).
\end{align}
}
...
\begin{align*}
 ...
 \brackets{\text{loss}} = & \brackets{\text{\small vanilla SAE loss}} + \alpha \sum_n b_n % q(b_n=1 \mid x)
\end{align*}
}
