...
\begin{figure*}[bt]
 \newcommand{\minilength}{0.35\textwidth}
...
\begin{table*}[tb]
 ...
 individual rows for the domains whose maximum pairwise coverage difference $\mit{MaxDiff}$
 (for the randomized configurations, we used the mean coverage of 10
 runs to calculate the coverage difference)
 $\geq 2$. (Details for domains with $\mit{MaxDiff}<2$ are in the
 supplemental material, \reftbl{lmcut-ipc-full}, \reftbl{lmcut-zerocost-full}.
 M\&S results are also in the supplemental material, \reftbl{mands-ipc-full}, \reftbl{mands-zerocost-full}).
