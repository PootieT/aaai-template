...
Downward \cite{Helmert2006}\footnote{http://www.fast-downward.org} uses a First-In-First-Out (FIFO) second-level tie-breaking policy. 
Although the use of FIFO tie-breaking in Fast Downward is repeatedly mentioned in \cite{RogerH10}, the reason for the particular choice of FIFO is not given.
...
LOP is negligeble.  We verified this by running a LOP
...
\begin{figure}[htb]
 ...
 \caption{Comparison of runtime on problems solved by both single FIFO
 search engine (ff) and a LOP engine with 2 different instances of the
 same FIFO engine (ffff). The runtime difference was on average below a
...
\begin{figure}[htb]
 ...
 \caption{Runtime comparson between LOP (X,Y,Z) and single strategies.}
