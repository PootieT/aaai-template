...
last layer of search accounts for a significant fraction of the effective search space of \astar.
...
This is because of operators with cost 0.
...
\begin{figure*}[htb]
 ...
 total \# of nodes in the search space (x-axis) with $f\leq f^*$ on 1104 IPC benchmark problems,
  ...
  See supplement for a labeled version (\refig{plateau-noh-full})
 ...
 \begin{minipage}[t]{\minilength}
  ...
  (larger color version with all labels in supplement: \refig{plateau-full}).
  }
  \label{plateau}
 \end{minipage} 
 \hfill
 \begin{minipage}[t]{0.26\textwidth}
  ...
  (larger color version with all labels in supplement: \refig{plateau-zerocost-full})
...
% evaluation order?}
We show that there are significant performances differences among tie-breaking strategies
when domains include zero-cost actions.  While there are relatively few domains with zero-cost actions in the existing IPC benchmark sets, we argue that this is a historical accident, and that in fact, many unit-cost domains may be more appropriately modeled using zero-cost actions.
...
\emph{Note that all tie-breaking strategies in this paper
...
which means: If there multiple nodes with the same $f$-value, first, break ties using $\text{criterion}_1$. 
...
tie-breaking, their library code ({\footnotesize https://github.com/eaburns/search})
...
\begin{table*}[htb]
 ...
 \caption{Coverage comparison (\# of instances solved in 5min, 2GB).
 ...
 (for the randomized strategies, we used the means of 10
 runs to calculate the $\mit{MaxDiff}$)
 $> 2$. (Details for domains with $\mit{MaxDiff}\leq 2$, and M\&S results,
 are in the supplemental material, Table \ref{lmcut-ipc-full}-\ref{mands-zerocost-full}).
 The following domains had $\mit{MaxDiff}\leq 2$:\\
 % 
...
\begin{figure}[tb]
 ...
 \caption{Comparisons of the \# of evaluations between simple \lifo and
 ...
 (larger color version with all labels in supplement: \refig{f-h-eval-full})
 }
 \label{f-h-eval}
\end{figure}
...
where (2), (3) follow from the fact that $f(n')=g(n')+h(n')$. %do not assume consistent heuristics.
...
This is similar to actively expanding nodes with low $h$-values, as done by $h$-based tiebreaking.
Therefore, the performance of $[\lifo]$ is comparable to that of $[h,\lifo]$.
...
runtime on these domains varies significantly depending on the second-level tie-breaking strategy.
...
objective to optimize the usage of one key consumable resource, e.g., fuel/energy cost minimization.
...
and made other actions to have zero cost. We did not
...
% fail to provide a meaningful guidance.
...
plateaus, i.e., tie-breaking,  becomes very important.
...
\reftbl{mands-ipc-full} and \reftbl{mands-zerocost-full} in the
...
$[h,\rd,\ro]$ is balancing the search at various depths, which results in resulting in successfully solving more problems within the time limit (\reftbl{depth}). %to avoid confusion, avoiding word ``coverage'' here because coverage could refer to search algorithm ``covering'' the search space.
% 
...
%We only reordered the actions because in the FD codebase, the order of propositions has no effect on tie-breaking. --- let's avoid talking about anything other than action names, in case there are other configuration factors.
...
%% criteria.
% %We avoided the effect of action orderings by using the randomized third tie-breaking.
...
($p=0.75$ for IPC, $p=0.26$ for Zerocost).
...
a direction for future work is development of  on-line adaptation methods that exploit search space neighborhood structure
in order to adjust the tie-breaking policies.
