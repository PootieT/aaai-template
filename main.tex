...
\refig{depth} shows various experiments on all domains including
...
These trend mostly applies to the cases where M\&S heuristics are used.
However, the trend is changed by the fact that it quickly exhaust the 2GB
memory limit.
...
\refig{portfolio-ff} to \refig{portfolio-r} shows the runtime between
...
node limited to a particular depth $k$. When
$k=0$, called $LA^*_0$ in their paper, the greedy expansion stops within
the same $f$-value. It is mostly the same as LastDepth strategy in our
case, but they did not realized that it is one case of tiebreaking strategy.

In the realm of inadmissible search, the work on escaping the plateau is
abundant.  Type based bucket \cite{xie14type} is an attempt to classify
the plateau of the greedy best first search according to the $[g,h]$
pair.  DBFS \cite{imai2011novel} is a technique which stochastically
backtracks GBFS to avoid being misdirected by the heuristic function.
