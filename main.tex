...
\begin{table*}[tb]
 ...
 the individual rows for the following domains whose maximum coverage difference $MaxCoverageDiff$
 (in randomized configuration, mean coverage of 10 runs)
 do not exceed 2 (Full results are in the supplemental material):\\
 % 
 (1) Domains with $MaxCoverageDiff = 0$ (same coverages by all configuration):
\pddls{
...
% of $[h,\ld,\ro]$, $[h,\rd,\ro]$,
using the non-parametric Wilcoxon's single-rank test.
