...
An issue in a vanilla SAE is that the class probability $\pi$ that is used inside its Gumbel-Softmax could be
...
This is because the former cares only about the accuracy of the output,
while the latter evaluates the quality of the \emph{hidden} activation values of the network while maintaining the same accuracy.
...
We compare the effectiveness of ZSAE with vanilla SAE in terms of variance reduction of the state encoding.
...
while the entropy regularization in vanilla SAE suppresses the stochastic behavior to some extent.
...
Results indicates that ZSAE is more robust on different latent space size and tend to solve more problems than vanilla SAE.
...
Similar results were obtained; ZSAE consistently improves the success ratio over the vanilla SAE.
