...
\todo{Tie-breaking or tiebreaking? Tie-Breaking.}
...
existing tiebreaking strategy for \astar.
% 
...
benchmark domains, and show the importance of tiebreaking strategy in
the more practical cost-minimization problems.

Next, in order to solve those cost-minimization problems,
we propose three tiebreaking methods
...
compared to the other tiebreaking strategies with the same heuristic function.

Also, we show the robustness of our tiebreaking strategies
...
planner performance is greatly affected by changing the action ordering
...
problems in this evaluation.  \reftbl{depth} shows the coverage results
...
The 3 rightmost columns in \reftbl{depth} show the $p$ values of the
test. The results depend on the domains, but in many situations we
observed a significant difference among 3 second-level tiebreaking, and
...
standard deviation of the coverage results of RandomDepth is smaller
than that of LastDepth, indicating the robustness of RandomDepth.
FirstDepth is dominated by the others, except in scanalyzer-analyse.

\todo{Any idea to improve this explanation?}
% removed \fifo
Finally, we answer two following concerns regarding \lifo final-level
tiebreaking criteria which we re-evaluated in the earlier sections and
now appear in $[f,h,\ld,\lifo]$ (and $[f,h,\lifo]$, which is basically a
same algorithm): First, the performance of \lifo could be affected by
the action orderings.
Second, using \lifo
could benefit from its efficient low-level memory access pattern (high
probability of hitting the cache) and its performance may not
corresponds to the search efficiency.
They are both rejected because the
coverage of $[f,h,\ld,\lifo]$ (and $[f,h,\lifo]$) is not significantly
...
% 
It means that the
performance of $[f,h,\ld,\lifo]$ was not caused by the third-level
\lifo, but instead by the second-level tiebreaking LastDepth.
...
\begin{table}[tb]
 \centering \relsize{-3}
 \input{tables/aaai16-robustnesscheck/aaai16prelim3_zerocost_2zerocost/1-7-rx-and-sig-wilcox-remove-same.tex}
 \caption{Comparison of the coverages of basic randomized queue with or
 without depth tiebreaking, $[f,h,\ro]$ and $[f,h,\rd,\ro]$. Each cell
...
\subsubsection{Do the Action Orderings Affect the Performance of Tiebreaking?}

Finally, we show that the performance of our depth-based tiebreaking is
...
original names of action schemas are uglyfied into a random name. This is because
...
We tested our most successful tiebreaking method $[f,h,\rd,\ro]$ on each
set of randomized domains, three times each with different random seeds,
resulting in 9 runs in total.
...
This technique explicitly targeted zero-cost actions observed in Openstacks,
and resulted in a significantly better performance in IPC-6 satisficing track.\todo{citation? -- add specific page+quotation in comment}
...
performing depth-based tiebreaking methods are LastDepth and RandomDepth
