...
\begin{figure*}[bt]
 ...
 \begin{minipage}[t]{0.23\textwidth}
  ...
  \caption{Similar to \refig{plateau}, but for 640 instances from our 
...
A \emph{plateau} is a set of open nodes with the same $f$ and same $h$.
...
and 640 problems from 29 \emph{zero-cost-action} domains (\refsec{sec:zerocost-domains}).
...
In \reftbl{depth}, which shows the results when  relying only on \fifo or \lifo tie-breaking.
...
\refig{depth-histogram} shows the histogram of the depth
...
\begin{figure}[tb]
 ...
 % \includegraphics{tables/aaai16-log-rd/2zerocost/depth-histogram-openstacks-p04.pdf}
 % \includegraphics{tables/aaai16-log-rd/2zerocost/depth-histogram-cybersec-p04.pdf}
...
\citeauthor{Hoffmann05} performed a detailed analysis of search space structures, focusing on  plateaus and dead ends which occur during local inadmissible search \cite{Hoffmann05,Hoffmann14}. 
% 
...
\citeauthor{RogerH10} (\citeyear{RogerH10}) considered 
the following tie-breaking approach,  which uses 2 heuristic functions.
One of the heuristics is used as the primary criterion for guiding the search,
...
a secondary heuristic as a tie-breaking criterion for cost-optimal planning
