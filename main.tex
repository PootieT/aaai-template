...
\label{loss+ent} =&\brackets{\text{\small the correct GS VAE loss}}      + 2 H(q(b \mid x_i)).
\end{align}
}
...
a much higher instability compared to the loss function with the negated KL-divergence (Eq.~\eqref{loss+ent}),
i.e., GS-VAE + Entropy regularization.
% 
...
for instances successfully solved by both SAE+AMA$_2$ and ZSAE($\alpha$=0.7+)AMA$_2$, both with $N$=100, all domains.
In both figures,
$x$-axis represent the SAE and $y$-axis represent the ZSAE.
...
Opposite cases are however generated, naturally due to
...
Our approach complements the usability of these approaches.
...
% 
The ZSAE improves the vanilla State AutoEncoder in Latplan by
