...
First, it comes from the way Gumbel distribution is sampled, $-\log (-\log (\text{Uniform}(0,1)))$,
which includes a uniform random distribution.
Thus, the propositional representation may change even for the exactly same inputs.
Second, the ``same'' input image may be corrupted by a random noise (stochastic observations).
...
because the duplicate detection in state space search algorithms (e.g. \astar) believes that they are separate.
...
for the same environment, under some definition of equivalence relation between environments.
\end{defi}
...
$-\sum_{i,j} q_{ij} \log \frac{1/M}{q_{ij}} = \sum_{i,j} q_{ij} (\log q_{ij} +\log M) = \sum_{i,j} q_{ij} \log q_{ij} + \text{Const.}$\,,
...
Despite that, the system works perfectly.
% 
