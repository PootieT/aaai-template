...
In \reftbl{depth}, we also show the results of $[\fifo]$ and $[\lifo]$,
...
on the \pddl{Woodworking-cut} p04 (left) and \pddl{psr-small-open} p48 (right) as an
interesting example.

In \pddl{Woodworking-cut} instances, we observed
...
depth, resulting in larger coverages (\reftbl{depth}).
% 
...
\pddl{Woodworkin-cut} p04 (left) and \pddl{psr-small-open} p48 (right) instances using
 $[h,\fd,\fifo]$, $[h,\ld,\lifo]$, $[h,\rd,\ro]$.
 $y$ axis is logarithmic. Large dots show where the solutions were found.
 }
