...
\begin{figure*}[bt]
 ...
 \caption{
 The number of the nodes with $f=f^*$ (y-axis) compared to the
 total number of nodes in the search space with $f\leq f^*$ on 1104 IPC benchmark problems
  (search space analyzed using Fast Downward with \lmcut heuristic,
  slightly modified to generate all nodes with cost $f^*$).
  Both axes are logarithmic. Dotted lines represent $\times 10^n$ boundaries.
  }
 ...
 \begin{minipage}[t]{\minilength}
  ...
  \caption{
  Same plot as \refig{fig:plateau-noh} except the $y$-axis showing
  the number of the nodes with $[f,h]=[f^*,0]$, which forms the final
  plateau when $h$ tie-breaking is enabled.
  We see many Openstacks and Cybersec instances near the $y=x$ line.
  We do not show all labels
  for point types, while supplemental material includes larger, fully
  labelled, colored variant.
  }
  \label{plateau}
 \end{minipage} 
 \hfill
 \begin{minipage}[t]{0.23\textwidth}
  ...
  \caption{Same plot as \refig{plateau}, but from the cost-minimization
...
\section{Tie-breaking Strategies in \astar}
 ...
 Although we could not find an explanation, this choice is most likely due to their use of alternating OPEN lists, in which case the \fifo policy serves to provide a limited form of fairness.
...
Therefore, the performance of $[f,\lifo]$ is comparable to the performance of $[f,h,\lifo]$.
...
domains are noticeable (e.g., \pddl{Cybersec}, Woodworking-cut). Also, the
...
\begin{table}[tb]
 \centering \relsize{-3}
 %\input{tables/aaai16-robustnesscheck/aaai16prelim3_zerocost_2zerocost/1-8-h-lf-vs-rd-random-noh.tex}
 \input{tables/aaai16-robustnesscheck/aaai16prelim3_zerocost_2zerocost/1-9-h-lf-vs-rd-random-noh-remove-same.tex}
