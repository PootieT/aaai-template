...
of steps a node is away from the entrance of the plateau.
...
tiebreaking, then $depth(n) = 0$. We call such $n$ the \emph{entrance} of the plateau.
...
We consider the following 3 tie-breaking configurations, each using $h$ as the first-level tie-breaking criterion, one \fd, \rd, \ld as the depth-based 2nd-level tie-breaking criterion, and \ro as the 3rd-level criterion:  $[f,h,\fd,\ro]$, $[f,h,\ld,\ro]$, $[f,h,\rd,\ro]$.
...
Although the node evaluation order of $[\cdot,\fd,\fifo]$ and $[\cdot,\ld,\lifo]$
...
Furthermore, \todo[we also tested the same domains with a reversed
action ordering.]{Running Experiments}.  The results (in supplimental
...
global cache for heuristic function values. Whenever a search
engine evaluates a state, it first checks the cache and reuses the
result if possible.  The current implementation is sequential -- each
search engine takes turns evaluating states. The algorithm finishes when
some engine finds the solution.
...
configurations does \refig{XXX}.\todo{??}
...
\begin{figure}[htb]
 \centering \relsize{-2}
 \includegraphics{tables/aaai16-30min/aaai16prelim3/time-lmcut_ff-lmcut_ffff.pdf}
 \caption{Comparison of the runtimes on problems solved by both single
 $[f,h,\fifo]$ (ff) and a LOP engine with 2 different instances of
 $[f,h,\fifo]$ (ffff). The runtime difference was on average below a
 factor of x1.1, if we ignore the subsecond differences.}  \label{ffff}
\end{figure}
...
%  \caption{Comparson of evaluations between LOP $[[\ld,\fifo],
%  [\rd,\lifo]]$ and the best single strategy $[\ld,\fifo]$. The results
%  unaffected by LOP are shown around the $y=2x$ line. }
...
\begin{figure}[htb]
 \centering
 \relsize{-2}
 \includegraphics{tables/aaai16-5min/aaai16prelim3/time-lmcut_m2-lmcut_ld_fifo.pdf}
 % \includegraphics{tables/aaai16-5min/aaai16prelim3/evaluated-nokey-lmcut_m2-lmcut_rd_lifo.pdf}
 \caption{Comparson of runtimes between LOP $[\ld,\fifo]$ +
 $[\rd,\lifo]$ and the best single strategy $[\ld,\fifo]$. The results
 most badly affected by LOP are shown around the $y=2x$ line. Most
 instances appear above the line, and many instances show that they
 avoided the worst case performance.}  \label{portfolio-runtime}
\end{figure}
...
\citeshort{RogerH10}. When combining two heuristics, one of the
...
Fast Downward and Lama implement an (OPEN) queue \emph{alternation} method \cite{Helmert2006}.
It was originally used as a method for using multiple heuristics in a planner.
There are multiple open lists (queues), where there is a queue for each heuristic, and states are inserted into all queues according to the heuristic value for each heuristic.
For example, if there are two heuristics $h1$ and $h2$, 
Our LOP portfolio approach is similar to queue alternation in that we maintain multiple queues, one for each tie-breaking method.
However, unlike queue alternation, TODO:INPROGRESS- kind of tricky to describe the difference succinctly..
...
Vallati et al \citeshort{vallati2015effective} investigated the effect of the ordering of objects (including operators, predicates, etc. ) in PDDL domain models, and showed that orderings can have a significant impact on planner performance.
