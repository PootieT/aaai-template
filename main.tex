...
This result is rather obvious, therefore we omit it due to space limitation (see supplement).
...
we applied a Gaussian noise to the image each time before we encode it.
The result in \reftbl{variance} indicates that
NG-SAE performs worst, and ZSAE performs best.


\reftbl{tab:stability} (first columns) shows that the propositions made by NG-SAE are highly random,
...
ZSAE further reduces the variance, as seen by the \emph{maximum} variance across propositions are near zero,
