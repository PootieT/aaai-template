...
experimentally analyzing the performance of commonly used tie-breaking strategies that break ties according to the heuristic value of the nodes.
We find that tie-breaking has a significant impact on search algorithm performance when there are actions with zero cost that induce large plateau regions in the search space.
We develop a new framework for tie-breaking based on a depth metric which measures distance from the entrance to the plateau, and propose a new, randomized strategy which significantly outperforms standard tie-breaking strategies on existing planning benchmark domains as well as newly generated domains with zero-cost actions.
...
for 
 $[h,\rd,\ro]$ %, modified to produce lots of outputs,
...
\pddl{Woodworkin-cut} instances. Solution can be found in variety of depths.
 }
...
When the final plateau of [f,h] is small, this will behave similarly to $[\rd,\ro]$.
...
resulting in 9 runs per method (recall that robustness wrto random seed was shown in \refsec{sec:depth-based-evaluation}.)
...
In contrast to the results for satisficing search by \cite{vallati2015effective}, 
the effect of domain configuration  seems to be relatively weak for cost-optimal search using \astar, at least when the domain configuration is limited to action renamining.
We conjecture that this is because 
the behavior of \astar with $[h,\lifo]$, $[h,\fifo]$, and $[h,rd,\ro]$ highly constrained due to the constraint that \astar strictly expands nodes in order of $f$, and furthermore, the 3 strategies we evaluated first break ties according to $h$, so
the effects of domain configuration is limited to 2nd (and higher) level tie-breaking.
...
\begin{table}[tb]
 ...
 and $[h,\rd,\ro]$ with three seeds. It contains 8 rows, each
 representing the original set of
 domains and its three action-reordered variants. The effect
