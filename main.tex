...
and address SSP in their preliminary implementation of \latentplanner,
specifically the uninformative / random propositions in the latent / symbolic encoding.
...
reasoning.
% 
To address SSP, we propose ``Zero-Suppressed SAE''
...
called ``symbol \emph{stability} problem'' (SSP), which seeks to find a set of stable symbols.
Stability is orthogonal to \emph{robustness}/\emph{generalization} which discuss accuracy,
...
\textbf{The stability of the representation and the \emph{robustness} against noisy data,
as is commonly listed as a virtue of neural networks, are orthogonal.}
...
on
the inherent stochasticity of the network during the runtime (as opposed to the training time) as well as
...
% The purpose of GS-VAE is to obtain a neural network
...
In practice, this is computed as follows.
...
to the latent, propositional representation, while preserving the reconstruction accuracy
 (\emph{robustness}, traditional machine-learning objective).
...
The KB no longer has to explicitly store the false propositions.
% 
...
(1) Vanilla SAE, as described in the original paper of Latplan \cite{Asai2018} and the Github source code,
(2) NG-SAE, an SAE trained with a correct GS-VAE loss function as discussed in \refsec{analysis}, and
...
after the training.
In all SAEs,
this significantly reduced the variance because it makes the network deterministic for a single input.
...
propositions), removing the need for hyperparameter tuning. The additional
