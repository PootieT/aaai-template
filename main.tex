...
Let $p(b \mid x)$ and $q(x \mid b)$ be the probabilities that the encoder and the decoder of GS-VAE output the value $b$ given $x$ and $x$ given $b$, respectively,
where $x$ corresponds to a visual observation of the environment, and $b\in\{0,1\}^N$ corresponds to its latent representation.
In principle, GS-VAE is trained by minimizing the following objective function with respect to $q(x \mid b)$ and $p(b\mid x)$:
\begin{align}
\label{vae-obj} \sum_{i=1}^I \mathbb{E}_{p(b\mid x_i)}\left[-\log q(x_i\mid b)\right] + \mathrm{KL}(p(b\mid x_i) \parallel p^\star(b)),
\end{align}
where $p^\star(b) = \prod_{n=1}^N p^\star(b_n) = \prod_{n=1}^N \mathrm{Bern}(0.5)$ is the target, $N$-dimensional Bernoulli distribution with uniform probabilities, 
...
$i$ is the index for each data point, and $\mathrm{KL}(X || Y)$ represents the Kullback-Leibler divergence from $Y$ to $X$.
...
and the second term regularizes the encoder by making the encoder $p(b\mid x)$ closer to $p^\star(b)$.
The second term is further computed as,
\begin{align*}
 &{\mathrm{KL}}(p(b\mid x_i) \parallel p^\star(b)) \\
= & - \sum_{n=1}^N \sum_{k\in\{0,1\}}p(b_{n}=k \mid x_i) \log\frac{p^\star(b_{n}=k)}{p(b_{n}=k \mid x_i)}\\
=& - \sum_{n=1}^N \sum_{k\in\{0,1\}}p(b_{n}=k \mid x_i) \left(\log \frac{1}{2} - \log{p(b_{n}=k \mid x_i)}\right)\\
=& - \sum_{n=1}^N H(p(b_n\mid x_i)) + \mathrm{const.} = -H(p(b \mid x_i)) + \mathrm{const.},
\end{align*}
...
It tries to make the distribution look more similar to a fair random categorical distribution $p^\star(b)$
...
instead of converging to any other random distribution different from $p^\star(b)$.
This can be shown as follows:
\begin{align} 
\nonumber  &\brackets{\text{rec loss}}-{\mathrm{KL}}(p||p^\star)                                    \\
\nonumber =&\brackets{\text{rec loss}}+{\mathrm{KL}}(p||p^\star) - 2 {\mathrm{KL}}(p||p^\star)          \\
\label{loss+ent} =&\brackets{\text{\small correct GS VAE loss}}      + 2 H(p(b \mid x_i)).
\end{align}
...
\begin{align*}
 ...
 =                        & \brackets{\text{\small vanilla SAE loss}} + \alpha \sum_n b_n % p(b_n=1 \mid x_i)
\end{align*}
...
% This technique alters $p^\star(b)$, the target distribution of GS-VAE.
% Recall that, in GS-VAE, $p^\star(b_n) = \mathrm{Bern}(0.5)$,
...
% Note that this generalizes to a categorical case by penalizing all non-zero values: $\bars{p(b_n \not= 0\mid x_i)}$
