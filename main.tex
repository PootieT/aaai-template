...
\relsize{-1}
\begin{align}
\label{vae-obj} \sum_{\mathbf{x}\in \mathcal{X}} \mathbb{E}_{q(\mathbf{b}\mid \mathbf{x})}\left[-\log p(\mathbf{x}\mid \mathbf{b})\right] + \mathrm{KL}(q(\mathbf{b}\mid \mathbf{x}) \parallel p(\mathbf{b})),
\end{align}
}
...
where $H_q(\mathbf{b}|\mathbf{x})$ is the conditional entropy of $\mathbf{b}$ given $\mathbf{x}$ under $q$.
...
Results indicate that ZSAE is more robust on different hyperparameters and tend to achieve better performance than vanilla SAE.
}
\label{tab:ama1}
\label{tab:ama2}
\end{table*}
