...
\end{align}
where $p_a(z) = \prod_{n=1}^N p_a(z_n) = \prod_{n=1}^N \mathrm{Bern}(0.5)$ is an $N$-dimensional Bernoulli distribution with uniform probabilities, and $i$ is the index for each data sample.
...
and the second term regularizes the encoder by making the encoder $q(z\mid x)$ closer to $p_a(z)$.
...
% similar to the $l_1$ norm (LASSO) typically applied to continuous activations, however it is 
\emph{asymmetric} to a particular label $z_n=1$:
\begin{align*}
 \brackets{\text{loss}} = & \brackets{\text{\small vanilla SAE loss}} + \brackets{\text{\small 0-Sup. loss}} \\ 
 =                        & \brackets{\text{\small vanilla SAE loss}} + \alpha \sum_n q(z_n = 1\mid x_i)
\end{align*}
...
% This technique alters $p_a(z)$, the target distribution of GS-VAE.
% Recall that, in GS-VAE, $p_a(z_n) = \mathrm{Bern}(0.5)$,
% i.e. the probability for $z_n$ to turn to 1 is a fair coin toss.
% Since zero-suppression penalizes the true bits ($z_n=1$), it changes this fairness: The larger the $\alpha$,
% the less $p_a(z_n=1)$,
...
% Note that this generalizes to a categorical case by penalizing all non-zero values: $\bars{q(z_n \not= 0\mid x_i)}$
