...
problems in this evaluation.  \refig{depth} shows the coverage results
on benchmark domains and zerocost domains. For randomized strategies, we
showed the mean and standard deviation of 10 individual runs.
...
which does not assume a normal distribution.  The 4 rightmost columns in
...
($=[f,h,\lifo]$) is not significantly different from $[f,h,\ld,\ro]$
($859 \in [863.5-8.9,863.5+8.9]$) despite the difference in the third-level
...
\begin{table}[tb]
 ...
 or did not pass the significance test under $p=0.05$ (randomized configuration).
