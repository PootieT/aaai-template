...
Fully automated high-level agents should be able to find symbolic representation
...
its applicability is limited to the tasks precisely modeled by human.
 % 
Meanwhile, \latentplanner \cite{Asai2018} uses a Deep Neural Network
called State AutoEncoder (SAE) to obtain a propositional
representation of the image based puzzle domains unsupervised,
and conducts Classical Planning on the propositional state space discovered,
partly resolving this bottleneck.

In this paper, we address a new sub-problem of symbol grounding problem, namely
the symbol \emph{stability} problem.  Informally, symbols are
\emph{stable} when they take the same value for the exact same
...
% 
To address this problem, we propose ``Zero-Suppressed SAE''
...
% Symbol Grounding problem is one of the key milestones in AI research
% which seeks to achieve high-level intelligence beyond reflex.
% In Physical Symbol Systems Hypothesis \cite{}, it is believed that
...
% development of highly optimized and generalized, domain-independent heuristics (e.g. FF \cite{Hoffmann01}, LMcut \cite{Helmert2009})
% that can be easily applied to multiple tasks with few or no data,
% while the current learning-based approaches struggle to improve its multi-task performance and data efficiency \cite{}.

Fully automated high-level agents should be able to find the symbolic
representation of an unknown environment by itself.  The task of
finding such a representation purely from observations is called Symbol
Grounding problem \cite{harnad1990symbol,Steels2008}.
