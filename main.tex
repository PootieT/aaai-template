...
=& - \sum_{n=1}^N H(q(b_n\mid \mathbf{x})) + \mathrm{const.} = -H(q(\mathbf{b} \mid \mathbf{x})) + \mathrm{const.},
\end{align*}
}
...
\label{loss+ent} =&\brackets{\text{\small the original GS VAE loss}}      + 2 H(q(\mathbf{b} \mid \mathbf{x})).
\end{align}
}
