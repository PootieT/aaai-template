...
Also note that Latplan is an established work published at AAAI18, not the workshop paper at KEPS17.



   Re: Significance.

We believe we raised an important and very broad problem regarding obtaining a symbolic model using machine learning.
We are glad that R1 has resonated with our message.

As a technical contribution, we theoretically analyzed the SAE and proposed a new regularization method for categorical latent representation.

While R3 stated that:
...
The effect of Entropy Regularization by SAE over NG-SAE on the variance is 1 to 2 orders of magnitude,
while the effect of Zero-Suppression is mostly 2 to 3, and up to 4 orders of magnitude (2.5e-4 -> 4.5e-8).
